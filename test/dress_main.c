...
static const char comname[LEN_NAME] = "D:\\04-data_collection20211213\\BLEdecode_exe_V4_withCMD\\DATA_DECODE\\ppg_RedIrGreen\\";
static const char * fileType = ".txt";
static const uint8_t version_num = 3;  // 1:version_4000  2:version_MeiXin  3；version_police
char sname1[LEN_NAME];
// 保存结果
char *stats_adrr = "C:\\Users\\cjl\\Desktop\\rel\\hr_stats.txt";
FILE *fp_stats = NULL;
...
void adbd_wear(uint16_t size, int32_t * pack_data);  //for test
				...
				if (file_n >= 0)  // 第几个文件
				{
					...
					if (fp_ppg)
					{
						...
						while (!feof(fp_ppg))   //文件不为空
						{
							...
							//	//*****每个采样点调用一次算法计算******
							//	judge_Iswearing(ir_data, &is_wear);
							//	printf("%d\n", is_wear);
							//	//***************************
							//	size -= 1;
							//}
...
/*测试代码*/
void adbd_wear(uint16_t size, int32_t * pack_data)
{
	uint8_t rel = 0;
	...
	while (size)
	{
		...
		//*****每个采样点调用一次算法计算******
		printf("%d", judge_wear_status());
		rel = judge_Iswearing(ir_data,red_data,-10);
		printf("%d ", rel);
		//***************************
		size -= 1;
	}
